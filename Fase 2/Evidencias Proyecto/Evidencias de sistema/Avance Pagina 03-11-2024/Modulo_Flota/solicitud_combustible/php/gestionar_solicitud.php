    ...
    try {
        if ($accion === 'aprobar') {
            ...
            if ($stmt_update->affected_rows >= 0) {
                // Obtener el nombre del usuario que aprobó la solicitud
                $sql_usuario = "SELECT nombre FROM usuario WHERE rut = ?";
                $stmt_usuario = $conexion->prepare($sql_usuario);
                $stmt_usuario->bind_param("s", $_SESSION['rut']);
                $stmt_usuario->execute();
                $resultado_usuario = $stmt_usuario->get_result();
                $usuario_aprobador = $resultado_usuario->fetch_assoc()['nombre'];
                ...
                if ($stmt_actualizar_km->affected_rows >= 0) {
                    // Insertar en el historial de kilometraje con el nombre del aprobador
                    $sql_historial = "INSERT INTO km_historial (patente, km, fecha_actualizacion, actualizado_por) 
                                      VALUES (?, ?, NOW(), ?)";
                    $stmt_historial = $conexion->prepare($sql_historial);
                    $stmt_historial->bind_param("sis", $patente, $nuevo_km, $usuario_aprobador);

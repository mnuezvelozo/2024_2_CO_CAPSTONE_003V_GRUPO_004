...
if (isset($_GET['patente'])) {
    ...
    $sql_km = "
    SELECT kh.km, kh.fecha_actualizacion, u.nombre AS actualizado_por
    FROM km_historial kh
    LEFT JOIN usuario u ON kh.actualizado_por = u.rut
    WHERE kh.patente = ?
    ORDER BY kh.id DESC
    LIMIT 10";
                        ...
                        <?php if ($resultado_historial_chofer->num_rows > 0): ?>
                            <?php while ($historial = $resultado_historial_chofer->fetch_assoc()): ?>
                                <tr>
                                    ...
                                    <td><?= htmlspecialchars($historial['kilometraje_inicio']) ?> Km</td>
                                    <td><?= $historial['fecha_termino'] ? htmlspecialchars($historial['fecha_termino']) : 'En curso' ?></td>
                                    <td><?= $historial['kilometraje_termino'] ? htmlspecialchars($historial['kilometraje_termino']) . ' Km' : 'En curso' ?></td>
                                </tr>
                            <?php endwhile; ?>
                        <?php else: ?>
                            <tr>

...
// Consulta 2: Kilometraje por vehículo
$sql_km_vehiculo = "SELECT v.patente, IFNULL(SUM(kh.km), 0) AS total_km
    FROM vehiculo v
    LEFT JOIN km_historial kh ON v.patente = kh.patente
    GROUP BY v.patente
    ORDER BY total_km desc;
    ";
$stmt_km_vehiculo = $conexion->prepare($sql_km_vehiculo);
$stmt_km_vehiculo->execute();
$resultado_km_vehiculo = $stmt_km_vehiculo->get_result();
$vehiculo_data = [];
while ($row_km_vehiculo = $resultado_km_vehiculo->fetch_assoc()) {
    $vehiculo_data[] = $row_km_vehiculo;
}
...
$sql_choferes_por_supervisor = "
    SELECT supervisor, COUNT(*) AS cantidad_choferes
    FROM usuario
...
<body>
    ...
    <div class="content">
        ...
        <!-- Gráfico y tabla de kilometraje por vehículo -->
        <div class="row">
            <div class="col-md-6">
                <div class="chart-container">
                    <h3>Kilometraje por Vehículo</h3>
                    <br>
                    <canvas id="kmVehiculoChart"></canvas>
                </div>
            </div>
            <div class="col-md-6">
                <br>
                <div class="vehiculo-table-responsive">
                    <?php require_once("tablas_reporteria/km_vehiculo.php") ?>
                </div>
            </div>
        </div>
        <hr>
    ...
    <?php require_once("graficos_reporteria/km_vehiculo.php") ?>

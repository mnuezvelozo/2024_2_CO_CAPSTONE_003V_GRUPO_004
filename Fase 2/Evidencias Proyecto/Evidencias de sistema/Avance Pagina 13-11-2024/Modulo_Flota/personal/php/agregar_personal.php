...
try {
    ...
    // Si el campo de supervisor está vacío, establecerlo como NULL
    if (empty($supervisor)) {
        $supervisor = null;
    }
    ...
    $sql_insertar = "
    INSERT INTO usuario (rut, nombre, fecha_ingreso, id_cargo, supervisor, activo)
    VALUES (?, ?, ?, ?, ?, 'Si')";

    $stmnt_insertar = mysqli_prepare($conexion, $sql_insertar);
    mysqli_stmt_bind_param($stmnt_insertar, "sssis", $rut, $nombre, $fecha_ingreso, $id_cargo, $supervisor);

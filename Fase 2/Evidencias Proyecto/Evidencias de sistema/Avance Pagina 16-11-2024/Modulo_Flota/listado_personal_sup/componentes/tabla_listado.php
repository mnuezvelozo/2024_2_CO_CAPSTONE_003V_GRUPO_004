...
// Obtener el RUT del supervisor logueado
$rut_supervisor = $_SESSION['rut'];

// Consulta para obtener el personal bajo el cargo del supervisor logueado
$sql_personal = "
    ...
    WHERE u.activo = 'Sí' AND u.supervisor = ?;  -- Filtrar solo usuarios activos bajo el supervisor logueado
";

$stmt_personal = $conexion->prepare($sql_personal);
$stmt_personal->bind_param("s", $rut_supervisor);
    ...
    <tbody>
        <?php foreach ($resultado_personal as $personal) : ?>
            <tr>
                ...
                <td><?= (new DateTime($personal['fecha_ingreso']))->format('d/m/Y') ?></td>

...
try {
    ...
    // Convertir "NULL" (string) a valor NULL real en PHP
    $nuevo_encargado = ($nuevo_encargado === "NULL") ? null : $nuevo_encargado;
    ...
    if ($rut_actual !== $nuevo_encargado) {
        ...
        // Crear un nuevo registro de historial para el nuevo chofer si no es NULL
        if (!is_null($nuevo_encargado)) {
            $sql_nuevo_historial = "
            ...
            // Generar una auditoría para el nuevo encargado
            $sql_nueva_auditoria = "
                INSERT INTO auditoria (patente, rut_responsable, fecha, estado) 
                VALUES (?, ?, CURDATE(), 'Pendiente')";
            $stmt_nueva_auditoria = $conexion->prepare($sql_nueva_auditoria);
            $stmt_nueva_auditoria->bind_param("ss", $patente, $nuevo_encargado);
            $stmt_nueva_auditoria->execute();
        }
    }

    // Actualizar el vehículo con el nuevo kilometraje y responsable (posiblemente NULL)
    $sql_actualizar = "
        UPDATE vehiculo 
    ...
    $conexion->commit();
    $icono = "success";
    $mensaje = "Actualizado correctamente y auditoría generada.";
} catch (Exception $e) {

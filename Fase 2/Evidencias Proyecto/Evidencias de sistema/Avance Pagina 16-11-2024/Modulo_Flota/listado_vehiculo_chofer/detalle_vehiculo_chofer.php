...
if (isset($_GET['patente'])) {
    ...
    $sql_historial_auditoria = "
    SELECT a.fecha, a.estado, u.nombre AS chofer, a.danos, a.documentos, a.funcionamiento, a.observaciones
    FROM auditoria a
    LEFT JOIN usuario u ON a.rut_responsable = u.rut
    WHERE a.patente = ?
    ORDER BY a.fecha DESC";

    $stmt_historial_auditoria = $conexion->prepare($sql_historial_auditoria);
    $stmt_historial_auditoria->bind_param("s", $patente);
    $stmt_historial_auditoria->execute();
    $resultado_historial_auditoria = $stmt_historial_auditoria->get_result();
} else {
...
<body>
    ...
    <nav class="sidebar">
        <div class="cuadricula">
            ...
            <a class="elemento sub" href="vehiculos_chofer.php">⏪ Regresar</a>
        </div>
        <!-- Dropdown Menú -->
        <div class="dropdown">
            <hr class="sidebarHr">
                                ...
                                <?php if (count($km_data) > 0): ?>
                                    <?php foreach ($km_data as $km): ?>
                                        <tr>
                                            ...
                                            <td><?= $km['km'] ? number_format($km['km'], 0, ',', '.') . ' km' : '' ?></td>
                        ...
                        <?php if ($resultado_historial_chofer->num_rows > 0): ?>
                            <?php while ($historial = $resultado_historial_chofer->fetch_assoc()): ?>
                                <tr>
                                    ...
                                    <td><?= $km['km'] ? number_format($historial['kilometraje_inicio'], 0, ',', '.') . ' km' : '' ?></td>
                    ...
                    <table class="table table-striped table-bordered">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th>Daños</th>
                                <th>Documentos</th>
                                <th>Funcionamiento luces/frenos</th>
                                <th>Observaciones</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php if ($resultado_historial_auditoria->num_rows > 0): ?>
                                <?php while ($auditoria = $resultado_historial_auditoria->fetch_assoc()): ?>
                                    <tr>
                                        <td><?= (new DateTime($auditoria['fecha']))->format('d/m/Y') ?></td>
                                        <td><?= htmlspecialchars($auditoria['estado']) ?></td>
                                        <td><?= htmlspecialchars($auditoria['chofer']) ?></td>
                                        <td><?= $auditoria['danos'] ? htmlspecialchars($auditoria['danos']) : 'Sin registro' ?></td>
                                        <td><?= $auditoria['documentos'] ? htmlspecialchars($auditoria['documentos']) : 'Sin registro' ?></td>
                                        <td><?= $auditoria['funcionamiento'] ? htmlspecialchars($auditoria['funcionamiento']) : 'Sin registro' ?></td>
                                        <td><?= $auditoria['observaciones'] ? htmlspecialchars($auditoria['observaciones']) : 'Sin observaciones' ?></td>
                                    </tr>
                                <?php endwhile; ?>
                            <?php else: ?>
                                <tr>
                                    <td colspan="7">No se ha registrado auditorías aún.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>

<div class="row">
    ...
    <div class="col-auto">
        <form action="php/excel_personal.php" method="POST">
...
$sql_personal = "
    ...
    u.supervisor AS rut_supervisor, COALESCE(s.nombre, 'N/A') AS nombre_supervisor
    FROM usuario u
                ...
                <td>
                    <div class="btn-group" role="group">
                        <button class="btn no-colapse" style="color: #cfbb4a" data-bs-toggle="modal" data-bs-target="#modal_actualizar"
                            onclick="completar_datos_modal_actualizar('<?= htmlspecialchars($personal['rut']) ?>', '<?= htmlspecialchars($personal['nombre']) ?>', '<?= htmlspecialchars($personal['fecha_ingreso']) ?>', '<?= htmlspecialchars($personal['id_cargo']) ?>', '<?= htmlspecialchars($personal['rut_supervisor']) ?>')">
...
<script>
    ...
    function completar_datos_modal_actualizar(rut, nombre, fecha_ingreso, id_cargo, rut_supervisor) {
        ...
        document.getElementById("input_supervisor").value = rut_supervisor;
    }
